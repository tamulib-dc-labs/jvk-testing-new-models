...
const DEFAULTS = {
  site: {
    ...
    theme: 'synthwave',
    repository: null // "owner/name"; powers the branch switcher
  },
  build: {
...
function getRepository(config) {
  // Priority: config file > GITHUB_REPOSITORY env var (set by Actions) > none.
  return config.site.repository || process.env.GITHUB_REPOSITORY || null;
}

/**
 * Emits public/site.json so the browser knows which repository to list
 * branches from. Without it the app falls back to inferring owner/repo from
 * the GitHub Pages URL, and to the config.json bundled with the build.
 */
function writeSiteInfo(config, basePath, repository) {
  const target = path.join(rootDir, 'public', 'site.json');
  const payload = {
    repository,
  ...
  console.log(`  Wrote public/site.json (repository: ${repository ?? 'auto-detect'})`);
}
...
function main() {
  ...
  console.log(`  Repository: ${repository ?? '(auto-detect from URL)'}`);
  console.log('');
  ...
  setupMediaConfig();
  writeSiteInfo(config, basePath, repository);
